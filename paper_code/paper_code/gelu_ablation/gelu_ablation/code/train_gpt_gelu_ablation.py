...
# ISOLATED GELU ablation script - completely separate from paper PReLU runs
# This script uses GELU MLP instead of Asymmetric PReLU MLP
#
# Purpose: Compare GELU vs PReLU for symmetry breaking experiments
# - Symmetric mode: No bQ, no bV, standard GELU MLP
# - bQ+bV mode: Full symmetry breaking in attention, standard GELU MLP
#
# DO NOT USE FOR PAPER RUNS - this is for ablation only
...
# --- Import ECD optimizer ---
# Add parent paths to import ECD
sys.path.insert(0, "/workspace/modded-nanogpt")
...
# -------------------------
# RoPE Implementation (same as PReLU version)
# -------------------------
def rotate_half(x):
    x1 = x[..., : x.shape[-1] // 2]
    x2 = x[..., x.shape[-1] // 2 :]
...
class FullSymmetryBrokenAttention(nn.Module):
    ...
    def __init__(self, config,
        ...
        # Q-K biases
        if symmetric:
            mean_Q = to_vec(0.0, 0.0)
            mean_K = to_vec(0.0, 0.0)
            std_Q  = to_vec(0.0, 0.0)
            std_K  = to_vec(0.0, 0.0)
        else:
...
# -------------------------
# GELU MLP (standard GPT-2 style) - THE KEY DIFFERENCE FROM PRELU VERSION
# -------------------------
class GELUMLP(nn.Module):
    """Standard GELU MLP as in GPT-2 (no asymmetric PReLU)"""
    def __init__(self, config):
        super().__init__()
...
class Block(nn.Module):
    def __init__(self, config, use_k_bias=True, use_v_bias=True,
                 mean_V=None, std_V=None, use_rope=False, use_flash=False, symmetric=False):
        super().__init__()
        self.sa = FullSymmetryBrokenAttention(
            ...
            use_rope=use_rope, use_flash=use_flash, symmetric=symmetric
        )
        # GELU MLP instead of Asymmetric PReLU
...
# -------------------------
# Robust FineWeb loader (same as PReLU version)
# -------------------------
...
def main():
    ...
    ap.add_argument("--adam_wd", type=float, default=0.0)
    ...
    ap.add_argument("--sgdm_init_velocity_std", type=float, default=0.1)
    ...
    # Symmetric mode (no bQ, no bV - standard attention)
    ...
    if args.use_flash and not FLASH_AVAILABLE:
        print("Warning: Flash Attention requested but not available.")
        args.use_flash = False

    if args.symmetric:
        args.use_v_bias = False
    ...
    # Print configuration
    print(f"=== GELU ABLATION (NOT FOR PAPER) ===")
    ...
    if args.symmetric:
        print(f"  Architecture: SYMMETRIC (no bQ, no bV)")
    else:
    ...
    print(f"  MLP: STANDARD GELU (no PReLU)")
    print(f"=====================================")
    ...
    # W&B setup
    if args.symmetric:
        symbreak_tag = "symmetric"
    else:
        symbreak_tag = "bQ"
        if args.use_v_bias:
            symbreak_tag += "+bV"
    pos_tag = "rope" if args.use_rope else "learned"
    attn_tag = "flash" if args.use_flash else "std"

    run_name = args.name or f"{args.model}-{args.optimizer}-gelu-{symbreak_tag}-{pos_tag}-seed{args.seed}"
    os.makedirs(args.log_dir, exist_ok=True)
    ckpt_dir = os.path.join(args.log_dir, run_name)
    os.makedirs(ckpt_dir, exist_ok=True)

    if args.wandb and WANDB_AVAILABLE:
        tags = [args.optimizer, args.model, symbreak_tag, pos_tag, attn_tag, "gelu-ablation"]
        wandb.init(
            project="gelu-ablation",  # Separate project for ablation
            name=run_name,
            tags=tags,
            config={
                ...
                "symmetric": args.symmetric,
                "mlp": "gelu",  # Key difference
                "symmetry_breaking": {
                    "bQ": not args.symmetric,
                    "bK": (not args.disable_k_bias) and (not args.symmetric),
                    "bV": args.use_v_bias and (not args.symmetric),
                    "mean_V": args.mean_V if (args.use_v_bias and not args.symmetric) else None,
                    "std_V": args.std_V if (args.use_v_bias and not args.symmetric) else None,
                }
            }
        )
    ...
    if args.optimizer == "ecd":
        ...
        adamw_kwargs = dict(lr=args.adam_lr, weight_decay=args.adam_wd, betas=(args.adam_beta1, args.adam_beta2))
        ...
        if args.sgdm_init_velocity_std > 0:
            with torch.no_grad():
                for group in opt.param_groups:
                    for p in group['params']:
                        state = opt.state[p]
                        state['momentum_buffer'] = torch.randn_like(p) * args.sgdm_init_velocity_std
        if args.wandb and WANDB_AVAILABLE:
    ...
    while consumed_tokens < args.train_tokens:
        ...
        if update % max(1, args.valid_every_updates) == 0:
            ...
            if val_loss < best_val_loss:
                ...
                torch.save({
                    ...
                    "mlp": "gelu",
                    "symmetry_breaking": {
                        "bQ": not args.symmetric, "bK": not args.disable_k_bias, "bV": args.use_v_bias,
        ...
        if args.save_every_updates and (update % args.save_every_updates == 0):
            ...
            torch.save({"model": model.state_dict(), "config": cfg, "update": update,
                        "consumed_tokens": consumed_tokens, "mlp": "gelu"}, path)
