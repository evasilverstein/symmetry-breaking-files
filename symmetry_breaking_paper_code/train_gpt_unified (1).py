...
# -------------------------
# SYMMETRIC Attention (Standard - no bQ)
# -------------------------
...
# -------------------------
# DISORDERED Attention (bQ symmetry breaking along with optional bV symmetry breaking)
# -------------------------
class DisorderedCausalSelfAttention(nn.Module):
    """
    Attention with Q-K symmetry breaking via bQ, and optional V-O breaking (bV).

    Note: bK is intentionally NOT included. While bK mathematically cancels out
    in attention (softmax normalizes over keys), we omit it for code clarity.
    """
    def __init__(self, config,
                 mean_Q=None, std_Q=None,
                 mean_V=None, std_V=None,
                 mode="per_batch", use_v_bias=False,
                 use_rope=False, use_flash=False):
        super().__init__()
        assert config.n_embd % config.n_head == 0
        self.n_head = config.n_head
        ...
        # Q bias (bK intentionally omitted - see class docstring)
        ...
        # Register Q buffers (bK intentionally omitted)
    ...
    def _resample_biases(self, device):
        # Resample bQ (bK intentionally omitted)
    ...
    def forward(self, x, rope_cos_sin=None):
        ...
        # Q symmetry breaking (bK intentionally omitted)
...
class SymmetricBlock(nn.Module):
    """Block with standard attention and GELU MLP (or PReLU MLP for architecture matching)."""
    def __init__(self, config, use_v_bias=False, mean_V=None, std_V=None,
                 use_rope=False, use_flash=False, use_prelu=False):
        super().__init__()
        self.sa = CausalSelfAttention(
            config, use_v_bias=use_v_bias, mean_V=mean_V, std_V=std_V,
            use_rope=use_rope, use_flash=use_flash
        )
        # Use PReLU MLP to match original architecture if requested
...
class DisorderedBlock(nn.Module):
    ...
    def __init__(self, config, use_v_bias=False,
...
"""
    ...
    ap.add_argument("--sgdm_init_velocity_std", type=float, default=0.0,
                    help="Initialize SGDM momentum buffers with random N(0, std) values (0 = zero init)")
    ...
    # Note: bK is intentionally not included (it cancels out in attention)
    ...
    # Architecture matching options
    ap.add_argument("--use_prelu", action="store_true", default=False,
                    help="Use PReLU MLP (learnable slopes) instead of GELU in symmetric mode. "
                         "Matches original train_scale_symmetric_ecd_soap.py architecture.")
    ...
    if args.symmetric:
        ...
        print(f"  Q-K sector: bQ=enabled (bK omitted)")
    ...
    if args.wandb and WANDB_AVAILABLE:
        ...
        wandb.init(
            ...
            config={
                ...
                "mlp": "prelu" if args.use_prelu else "gelu",
                "symmetry_breaking": {
                    "symmetric": args.symmetric,
                ...
                "sgdm_init_velocity_std": args.sgdm_init_velocity_std if args.optimizer == "sgdm" else None,
            }
        )
    ...
    if args.optimizer == "ecd":
        ...
        # Initialize momentum buffers with random values if requested
        if args.sgdm_init_velocity_std > 0:
            print(f"  Initializing SGDM momentum buffers with std={args.sgdm_init_velocity_std}")
