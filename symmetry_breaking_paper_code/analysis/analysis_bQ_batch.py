...
def load_model_and_config(ckpt_path, device, f):
    ...
    # Model size presets (from train_scale_disorder_ecd.py)
    MODEL_PRESETS = {
    ...
    if isinstance(config_dict, dict):
        # Check if it's a training config with 'model' key (e.g., '124m')
    ...
    if 'model' in ckpt:
        ...
        raise KeyError(f"Checkpoint has neither 'model' nor 'model_state_dict'. Keys: {list(ckpt.keys())}")
